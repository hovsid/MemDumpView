 ...
 * Each row:
 *  - has a checkbox to select (calls onSelect)
 *  - clicking the row toggles the pin.hidden flag (calls onHide)
 *  - has a menu button that opens a floating menu (重命名 / 删除) appended to document.body
 *
 * Callbacks to set from consumer:
 *  - this.onDelete(p)
 *  - this.onSelect(p, ev)
 *  - this.onHide(p, ev)

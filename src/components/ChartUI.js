...
export class ChartUI {
  ...
  _onWheel(ev) {
    ...
    const span = Math.max(1, this.core.viewMaxX - this.core.viewMinX || 1);
  ...
  handleKeyEvent(ev) {
    ...
    if (key === 'a') {
      ev.preventDefault();
      this.core.viewMinX = Math.max(0, this.core.viewMinX - panStep);
      this.core.viewMaxX = this.core.viewMinX + span;
      this.core.resampleInView();
    } else if (key === 'd') {
      ev.preventDefault();
      const ext = this.core.computeGlobalExtents();
      this.core.viewMinX = Math.min(Math.max(0, ext.max - span), this.core.viewMinX + panStep);
      this.core.viewMaxX = this.core.viewMinX + span;
      this.core.resampleInView();
    } else if (key === 'w') {
      ev.preventDefault();

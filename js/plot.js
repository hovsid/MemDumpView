...
export function renderHeapPlot(state){
  const xLine = state.dsActive ? state.dsCurrentX : Array.from({length: state.heapValuesOriginal.length}, (_,i) => i+1);
  const yLine = state.dsActive ? state.dsCurrentY : state.heapValuesOriginal;

  const actualTrace = {
    x: xLine, y: yLine, type: 'scatter', mode: 'lines',
    name: state.dsActive ? `Heap Bytes (downsampled ${state.dsCurrentX.length})` : 'Heap Bytes',
    line: { color: '#1976d2', width: 2 }
  };

  const markerTrace = {
    x: state.heapGcMarkers, y: state.heapGcMarkerValues, type: 'scatter', mode: 'markers',
    name: 'GC events',
    marker: { symbol: 'star-diamond', size: 9, color: 'red', line: { color: '#000', width: 1 } },
    text: state.heapGcMarkers.map((idx,i) => `GC event #${i+1} @ sample ${idx}<br>Heap: ${state.heapGcMarkerRawValues[i]}`),
    hoverinfo: 'text'
  };
  ...
  if(state.haveSimulation && state.simulatedCompactedX.length){
    traces.push({
      ...
      text: state.simulatedCompactedY.map((v,i)=>`Simulated after GC#${i+1} @ sample ${state.simulatedCompactedX[i]}<br>${formatBytes(v)}`),
      hoverinfo: 'text'
    });
  }
...
export function highlightHeapMarkerByPosition(state, pos){
  ...
  const ann = {
    x: state.heapGcMarkers[pos], y: state.heapGcMarkerValues[pos],

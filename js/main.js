...
function renderGCPairs(){
  ...
  const sortedPairIndices = state.gcPairs.map(p=>p.idx).sort((a,b)=>a-b);
  let simMap = new Map();
  ...
  for(const pair of state.gcPairs){
    ...
    summaryPanel.querySelector('.highlight-btn').onclick = () => { Plot.highlightAndFocusHeapMarker(state, pair.idx); Plot.jumpToHeapTimeline(); };
  ...
  state.simulatedCompactedX = [];
  state.simulatedCompactedY = [];
  for(let pos=0; pos<state.heapGcMarkers.length && pos<sortedIndices.length; pos++){
...
function buildCorrelationPanel(){
  ...
  if(gcIndices.length && state.heapGcMarkers.length){
    ...
    gcIndices.forEach(idx => { html += `<div class="corr-row" data-gc="${idx}">GC ${idx}</div>`; });
  } else {
    html += "<div><b>Heap GC markers:</b></div>";
    state.heapGcMarkers.forEach((s,i) => { html += `<div class="corr-row" data-marker="${i}">Marker ${i+1} @ sample ${s}</div>`; });
  }
...
document.getElementById('merged-input').addEventListener('change', e=>{
  ...
  reader.onload = ev => { parseMergedFile(ev.target.result); };
...
document.getElementById('apply-ds-btn').addEventListener('click', ()=>{
  ...
  if(state.heapValuesOriginal.length === 0){ alert("Load merged file first."); return; }
